...
import type { OpmlFeed, OpmlFile, StorageFile } from '../models';
...
export class OPML {
  ...
  public async save(): Promise<void> {
    if (!this.data || !this.originalData) throw new Error('You must open a file first');
  ...
  static async create(filePathAndName?: string, feeds: OpmlFeed[] = []): Promise<OpmlFile> {
    ...
    await this._saveFile(filePathAndName || defaultFilePath, this._createBlob(file));

    return file;
  }

  private static async _openAndParseFile(filePathAndName: string): Promise<OpmlFile> {
    const file = await this._openFile(filePathAndName);
    const fileText = await this._readFileAsText(file);
    const xml = new DOMParser().parseFromString(fileText, 'text/xml');
  ...
  private static async _openFile(filePathAndName: string): Promise<File> {
    if (!(navigator as any).getDeviceStorage) {
      ...
      const exampleData = await fetch('assets/example.opml').then((res) => res.blob());
    ...
    const xmlString = `
      ...
      <opml version="2.0">
        <head>
          ...
          ${file.dateCreated ? `<dateCreated>${file.dateCreated}</dateCreated>` : ''}
          ${file.dateModified ? `<dateModified>${file.dateModified}</dateModified>` : ''}
        </head>
        <body>
          ${file.feeds.reduce((acc, feed) => {
            let outline = '<outline ';
            if (feed.text) outline += `text="${this._encodeString(feed.text)}" `;
            if (feed.xmlUrl) outline += `xmlUrl="${this._encodeString(feed.xmlUrl)}" `;
            if (feed.type) outline += `type="${this._encodeString(feed.type)}" `;
            if (feed.description)
              outline += `description="${this._encodeString(feed.description)}" `;
            if (feed.htmlUrl) outline += `htmlUrl="${this._encodeString(feed.htmlUrl)}" `;
            if (feed.language) outline += `language="${this._encodeString(feed.language)}" `;
            if (feed.title) outline += `title="${this._encodeString(feed.title)}" `;
            if (feed.version) outline += `version="${this._encodeString(feed.version)}" `;
